...
export default class Main extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      btnModel: require('../../../utils/img/15.png'), //播放模式按钮背景图
      pic_small: '', //小图
      pic_big: '', //大图
      ...
      isplayBtn: require('../../../utils/img/18.png'), //播放/暂停按钮背景图
    };
  }
  //上一曲
  prevAction = (index) => {
  ...
  playModel = (playModel) => {
    ...
    if (playModel == 1) {
      this.setState({
        btnModel: require('../../../utils/img/15.png'),
      });
    } else if (playModel == 2) {
      this.setState({
        btnModel: require('../../../utils/img/17.png'),
      });
    } else {
      this.setState({
        btnModel: require('../../../utils/img/16.png'),
      });
    }
  };
  //播放/暂停
  playAction = () => {
    ...
    if (this.state.pause == true) {
      this.setState({
        isplayBtn: require('../../../utils/img/18.png'),
      });
    } else {
      this.setState({
        isplayBtn: require('../../../utils/img/14.png'),
      });
    }
  };
  //播放器每隔250ms调用一次
  ...
  render() {
    ...
    if (this.state.file_link.length <= 0) {
      return <ActivityIndicator animating={this.state.animating} style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }} size="large" />;
    } else {
      ...
      return (
        <View style={styles.container}>
          ...
          <View style={{ position: 'absolute', width: width }}>
            {/*胶片光盘*/}
            <Image source={require('../../../utils/img/22.gif')} style={{ width: 300, height: 250, alignSelf: 'center' }} />
            ...
            <View style={styles.playingInfo}>
              ...
              {/*时间*/}
              <Text>
                {this.formatTime(Math.floor(this.state.currentTime))} - {this.formatTime(Math.floor(this.state.duration))}
              </Text>
            </View>
            {/*播放模式*/}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around' }}>
              <TouchableOpacity onPress={() => this.prevAction(this.state.currentIndex - 1)}>
                <Image source={require('../../../utils/img/20.png')} style={{ width: 30, height: 30 }} />
              </TouchableOpacity>
              ...
              <TouchableOpacity onPress={() => this.nextAction(this.state.currentIndex + 1)}>
                <Image source={require('../../../utils/img/21.png')} style={{ width: 30, height: 30 }} />
              </TouchableOpacity>
            </View>

            {/*歌词*/}
            <View style={{ height: 140, backgroundColor: 'red', alignItems: 'center' }}>
              <ScrollView
